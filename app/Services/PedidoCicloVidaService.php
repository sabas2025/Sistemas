...
class PedidoCicloVidaService {
  ...
  public static function receberRetornoVsm(array $payload, ?string $xmlRaw=null): array {
    ...
    try { self::pdo()->prepare('UPDATE pedidos_hub SET pedido_vsm_id=COALESCE(NULLIF(?,\'\'),pedido_vsm_id), status_vsm=?, data_recebido_vsm=NOW(), atualizado_em=NOW() WHERE id=?')->execute([(string)($payload['pedido_vsm_id'] ?? $payload['id'] ?? ''),$statusVsm,$pedidoId]); } catch(Throwable $e){ if (class_exists('BestEffortLogService')) BestEffortLogService::warning(__METHOD__, $e); }

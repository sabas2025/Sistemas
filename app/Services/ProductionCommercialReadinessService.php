...
class ProductionCommercialReadinessService {
  public static function run(): array {
    ...
    $add('Tenant','Escopo multiempresa/multifilial', !empty($cfg['commercial']['tenant_scope_required']) ? 'ok':'alerta', !empty($cfg['commercial']['tenant_scope_required']) ? 'Escopo obrigatório ligado.' : 'Escopo obrigatório ainda em monitoramento.', 'P0-02 (reauditoria 2026-08-23): NÃO ligar tenant_scope_required para operar múltiplos clientes reais ainda - o filtro por empresa/filial (appendWhereIfColumns) não é usado em nenhuma consulta de Pedido/Produto/Fila hoje. Ligar a flag sem isso apenas bloqueia rotas operacionais sem empresa selecionada; não isola dados entre clientes.');

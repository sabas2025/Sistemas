...
class SecurityAssistedTestService {
  ...
  private static function checksCommercialReadiness(): array {
    ...
    $checks[] = self::check('tenant_scope', 'Escopo multiempresa/multifilial configurável', class_exists('TenantContextService'), 'medio', 'P0-02: NÃO ative tenant_scope_required em produção multicliente ainda - as queries de Pedido/Produto/Fila não filtram por empresa/filial (appendWhereIfColumns não é chamado em nenhum lugar). Hoje isto só bloqueia acesso a rotas operacionais sem empresa selecionada, não isola dados entre clientes.', ['tenant_scope_required'=>!empty($commercial['tenant_scope_required'])]);

-- Baseline histórico V104.36 compatibilizado com V104.49.3-R5 (2026-08-22)
...
-- schema_migrations já foi declarada acima com o contrato compatibilizado.
...
-- V104.36
SET @hub_col_exists := (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='integration_idempotency' AND column_name='owner_token');
SET @hub_col_sql := IF(@hub_col_exists=0, 'ALTER TABLE integration_idempotency ADD COLUMN owner_token CHAR(64) NULL AFTER original_fila_id', 'SELECT 1');
PREPARE hub_col_stmt FROM @hub_col_sql; EXECUTE hub_col_stmt; DEALLOCATE PREPARE hub_col_stmt;
SET @hub_col_exists := (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='integration_idempotency' AND column_name='locked_until');
SET @hub_col_sql := IF(@hub_col_exists=0, 'ALTER TABLE integration_idempotency ADD COLUMN locked_until DATETIME NULL AFTER owner_token', 'SELECT 1');
PREPARE hub_col_stmt FROM @hub_col_sql; EXECUTE hub_col_stmt; DEALLOCATE PREPARE hub_col_stmt;
SET @hub_idx_exists := (SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema=DATABASE() AND table_name='integration_idempotency' AND index_name='idx_idemp_lock');
SET @hub_idx_sql := IF(@hub_idx_exists=0, 'CREATE INDEX idx_idemp_lock ON integration_idempotency(status, locked_until)', 'SELECT 1');
PREPARE hub_idx_stmt FROM @hub_idx_sql; EXECUTE hub_idx_stmt; DEALLOCATE PREPARE hub_idx_stmt;
SET @hub_col_exists := (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='fila_integracao' AND column_name='lease_expires_at');
SET @hub_col_sql := IF(@hub_col_exists=0, 'ALTER TABLE fila_integracao ADD COLUMN lease_expires_at DATETIME NULL AFTER locked_at', 'SELECT 1');
PREPARE hub_col_stmt FROM @hub_col_sql; EXECUTE hub_col_stmt; DEALLOCATE PREPARE hub_col_stmt;
SET @hub_col_exists := (SELECT COUNT(*) FROM information_schema.columns WHERE table_schema=DATABASE() AND table_name='fila_integracao' AND column_name='heartbeat_at');
SET @hub_col_sql := IF(@hub_col_exists=0, 'ALTER TABLE fila_integracao ADD COLUMN heartbeat_at DATETIME NULL AFTER lease_expires_at', 'SELECT 1');
PREPARE hub_col_stmt FROM @hub_col_sql; EXECUTE hub_col_stmt; DEALLOCATE PREPARE hub_col_stmt;
SET @hub_idx_exists := (SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema=DATABASE() AND table_name='fila_integracao' AND index_name='idx_fila_lease');
SET @hub_idx_sql := IF(@hub_idx_exists=0, 'CREATE INDEX idx_fila_lease ON fila_integracao(status, lease_expires_at)', 'SELECT 1');
PREPARE hub_idx_stmt FROM @hub_idx_sql; EXECUTE hub_idx_stmt; DEALLOCATE PREPARE hub_idx_stmt;

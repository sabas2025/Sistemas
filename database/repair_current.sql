-- V104.49.3-R5 (2026-08-22) - Reparo não destrutivo para banco único.
-- Execute no banco principal do HUB quando o SchemaGuard não puder criar via painel.
-- Este arquivo não cria/altera usuários e não sobrescreve credenciais,
-- endpoints, ambientes ou configurações operacionais existentes.
-- ATENÇÃO: além do DDL, contém INSERTs idempotentes de catálogo, permissões e
-- histórico de migrations. O AutoRepair do painel ignora esses DMLs; na execução
-- manual eles são aplicados sem substituir linhas existentes.

-- Hub Tiny VSM - reparo consolidado V104.49.3-R5
-- Schema oficial limpo e compatível com MySQL/MariaDB.
-- Evita alterações incompatíveis; colunas novas já estão nas tabelas ou são reparadas pelo SchemaGuard.
...
-- V104.49.3-R5: preserva dados e completa instalações anteriores.
SET @hub_sql := IF((SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='configuracoes_integracao' AND COLUMN_NAME='vsm_ambiente')=0, 'ALTER TABLE configuracoes_integracao ADD COLUMN vsm_ambiente VARCHAR(30) DEFAULT ''homologacao''', 'SELECT 1');
PREPARE hub_stmt FROM @hub_sql; EXECUTE hub_stmt; DEALLOCATE PREPARE hub_stmt;
SET @hub_sql := IF((SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='configuracoes_integracao' AND COLUMN_NAME='vsm_producao_liberada')=0, 'ALTER TABLE configuracoes_integracao ADD COLUMN vsm_producao_liberada TINYINT DEFAULT 0', 'SELECT 1');
PREPARE hub_stmt FROM @hub_sql; EXECUTE hub_stmt; DEALLOCATE PREPARE hub_stmt;
SET @hub_sql := IF((SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='configuracoes_integracao' AND COLUMN_NAME='vsm_ultimo_teste_ok')=0, 'ALTER TABLE configuracoes_integracao ADD COLUMN vsm_ultimo_teste_ok TINYINT DEFAULT 0', 'SELECT 1');
PREPARE hub_stmt FROM @hub_sql; EXECUTE hub_stmt; DEALLOCATE PREPARE hub_stmt;
SET @hub_sql := IF((SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='configuracoes_integracao' AND COLUMN_NAME='vsm_ultimo_teste_em')=0, 'ALTER TABLE configuracoes_integracao ADD COLUMN vsm_ultimo_teste_em DATETIME NULL', 'SELECT 1');
PREPARE hub_stmt FROM @hub_sql; EXECUTE hub_stmt; DEALLOCATE PREPARE hub_stmt;
SET @hub_sql := IF((SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='configuracoes_integracao' AND COLUMN_NAME='vsm_host_producao_liberado')=0, 'ALTER TABLE configuracoes_integracao ADD COLUMN vsm_host_producao_liberado VARCHAR(255) NULL', 'SELECT 1');
PREPARE hub_stmt FROM @hub_sql; EXECUTE hub_stmt; DEALLOCATE PREPARE hub_stmt;
SET @hub_sql := IF((SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='configuracoes_integracao' AND COLUMN_NAME='vsm_producao_liberada_em')=0, 'ALTER TABLE configuracoes_integracao ADD COLUMN vsm_producao_liberada_em DATETIME NULL', 'SELECT 1');
PREPARE hub_stmt FROM @hub_sql; EXECUTE hub_stmt; DEALLOCATE PREPARE hub_stmt;
SET @hub_sql := IF((SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='configuracoes_integracao' AND COLUMN_NAME='vsm_producao_liberada_por')=0, 'ALTER TABLE configuracoes_integracao ADD COLUMN vsm_producao_liberada_por INT NULL', 'SELECT 1');
PREPARE hub_stmt FROM @hub_sql; EXECUTE hub_stmt; DEALLOCATE PREPARE hub_stmt;
...
-- Reparo estrutural não cria nem altera usuários. A conta administrativa deve
-- ser provisionada pelo instalador autenticado, com hash gerado em tempo real.

-- Reparo estrutural também não cria dados de empresa/filial do tenant.
...
-- Reparo estrutural preserva integralmente as configurações e credenciais já
-- cadastradas. Defaults operacionais são responsabilidade do instalador novo.
...
-- Nenhum seed de configuração é executado durante o reparo. Isso preserva
-- tokens, secrets, endpoints, ambientes e decisões operacionais existentes.
...
-- V57: defaults pertencem ao fluxo de instalação nova, não ao reparo.
...
-- V104.8: tabelas runtime/core incluídas no instalador modular para evitar criação tardia em tela.
...
-- Configurações de estoque existentes não são semeadas nem sobrescritas pelo reparo.
...
-- O reparo não cria nem ativa agendamentos operacionais.
...
-- A fonte real e os fluxos de estoque continuam exatamente como configurados.
...
-- Consulta VSM não é ativada nem reconfigurada por um reparo de schema.
...
-- Consulta Tiny não é alterada por um reparo de schema.
...
-- V104.22 - Compatibilidade Backups arquivo/nome_arquivo/caminho aplicada na definição oficial de backups_banco.
...
-- Valores de ambiente e versão não são normalizados automaticamente. O reparo
-- altera apenas o tipo das colunas e preserva o conteúdo cadastrado.

INSERT INTO schema_migrations(migration, checksum, status, mensagem)
VALUES('v104_24_autorepair_config_db_storage_mode', SHA2('v104_24_autorepair_config_db_storage_mode', 256), 'aplicada', 'Compatibilidade estrutural de ambiente/tiny_versao aplicada sem alterar valores operacionais.')
ON DUPLICATE KEY UPDATE status='aplicada', mensagem=VALUES(mensagem), aplicada_em=CURRENT_TIMESTAMP;
...
-- V104.34 - Registro de bootstrap Enterprise Core.
INSERT INTO schema_migrations(migration, checksum, status, mensagem) VALUES('v104_34_enterprise_core_bootstrap', SHA2('v104_34_enterprise_core_bootstrap', 256), 'aplicada', 'Bootstrap Enterprise Core disponível na instalação atual.') ON DUPLICATE KEY UPDATE status='aplicada', mensagem=VALUES(mensagem), aplicada_em=CURRENT_TIMESTAMP;
...
-- schema_migrations já foi declarada acima com o contrato canônico atual.
...
SET @hub_sql := IF((SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='fila_integracao' AND COLUMN_NAME='idempotency_key')=0, 'ALTER TABLE fila_integracao ADD COLUMN idempotency_key VARCHAR(190) NULL', 'SELECT 1');
PREPARE hub_stmt FROM @hub_sql; EXECUTE hub_stmt; DEALLOCATE PREPARE hub_stmt;
SET @hub_sql := IF((SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='fila_integracao' AND COLUMN_NAME='locked_by')=0, 'ALTER TABLE fila_integracao ADD COLUMN locked_by VARCHAR(120) NULL', 'SELECT 1');
PREPARE hub_stmt FROM @hub_sql; EXECUTE hub_stmt; DEALLOCATE PREPARE hub_stmt;
SET @hub_sql := IF((SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='fila_integracao' AND COLUMN_NAME='locked_at')=0, 'ALTER TABLE fila_integracao ADD COLUMN locked_at DATETIME NULL', 'SELECT 1');
PREPARE hub_stmt FROM @hub_sql; EXECUTE hub_stmt; DEALLOCATE PREPARE hub_stmt;
SET @hub_idx_exists := (SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema=DATABASE() AND table_name='fila_integracao' AND index_name='idx_fila_idempotency_key');
SET @hub_idx_sql := IF(@hub_idx_exists=0, 'CREATE INDEX idx_fila_idempotency_key ON fila_integracao(idempotency_key)', 'SELECT 1');
PREPARE hub_idx_stmt FROM @hub_idx_sql; EXECUTE hub_idx_stmt; DEALLOCATE PREPARE hub_idx_stmt;
SET @hub_idx_exists := (SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema=DATABASE() AND table_name='fila_integracao' AND index_name='idx_fila_status_proxima_prioridade');
SET @hub_idx_sql := IF(@hub_idx_exists=0, 'CREATE INDEX idx_fila_status_proxima_prioridade ON fila_integracao(status, proxima_tentativa, prioridade, id)', 'SELECT 1');
PREPARE hub_idx_stmt FROM @hub_idx_sql; EXECUTE hub_idx_stmt; DEALLOCATE PREPARE hub_idx_stmt;
SET @hub_idx_exists := (SELECT COUNT(*) FROM information_schema.statistics WHERE table_schema=DATABASE() AND table_name='fila_integracao' AND index_name='idx_fila_locked');
SET @hub_idx_sql := IF(@hub_idx_exists=0, 'CREATE INDEX idx_fila_locked ON fila_integracao(locked_by, locked_at)', 'SELECT 1');
PREPARE hub_idx_stmt FROM @hub_idx_sql; EXECUTE hub_idx_stmt; DEALLOCATE PREPARE hub_idx_stmt;

-- integration_events e integration_idempotency já foram declaradas acima.
-- Os ALTERs seguintes completam apenas colunas ausentes em bases antigas.

SET @hub_sql := IF((SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='integration_idempotency' AND COLUMN_NAME='claim_count')=0, 'ALTER TABLE integration_idempotency ADD COLUMN claim_count INT DEFAULT 0', 'SELECT 1');
PREPARE hub_stmt FROM @hub_sql; EXECUTE hub_stmt; DEALLOCATE PREPARE hub_stmt;
SET @hub_sql := IF((SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='integration_idempotency' AND COLUMN_NAME='last_duplicate_at')=0, 'ALTER TABLE integration_idempotency ADD COLUMN last_duplicate_at DATETIME NULL', 'SELECT 1');
PREPARE hub_stmt FROM @hub_sql; EXECUTE hub_stmt; DEALLOCATE PREPARE hub_stmt;
SET @hub_sql := IF((SELECT COUNT(*) FROM information_schema.COLUMNS WHERE TABLE_SCHEMA=DATABASE() AND TABLE_NAME='integration_idempotency' AND COLUMN_NAME='original_fila_id')=0, 'ALTER TABLE integration_idempotency ADD COLUMN original_fila_id BIGINT NULL', 'SELECT 1');
PREPARE hub_stmt FROM @hub_sql; EXECUTE hub_stmt; DEALLOCATE PREPARE hub_stmt;

...
<div class="col-12"><section id="sec-vsm" class="integration-card vsm-card mb-3"><div class="integration-card-title"><div><h2><i class="bi bi-diagram-3"></i> VSM / Conecta Venda</h2><p>Para o HUB Tiny ⇄ VSM, use <b>pedidos-integradora</b> como API principal. O Swagger <b>pedidos-loja</b> fica opcional e só deve ser usado se a VSM solicitar endpoint específico de loja.</p></div><span class="integration-card-badge">VSM principal</span></div><?php $vsmSt = $config['vsm_status_operacional'] ?? ['modo'=>'Homologação','detalhe'=>'URL de homologação configurada','acao'=>'Produção bloqueada até liberação manual']; ?><div class="alert alert-warning border small"><b>Status operacional VSM:</b> <?=e($vsmSt['modo'])?> — <?=e($vsmSt['detalhe'])?>.<br><span><?=e($vsmSt['acao'])?></span></div><div class="row g-3">
  ...
  <div class="col-md-4"><label class="form-label">Client Token (Integradora)</label><input class="form-control" name="vsm_client_token" value="<?=e(Secrets::mask($config['vsm_client_token']??''))?>"><div class="form-text">Autentica em <code>/v1/auth/token</code> e vai como <code>clientTokenIntegradora</code> na query do pedido.</div></div>
  <div class="col-md-4"><label class="form-label">Client Secret (Integradora)</label><input class="form-control" name="vsm_client_secret" value="<?=e(Secrets::mask($config['vsm_client_secret']??''))?>"><div class="form-text">Segredo da integradora, usado só em <code>/v1/auth/token</code>.</div></div>
  <div class="col-md-4"><label class="form-label">Client Token (Loja)</label><input class="form-control" name="vsm_client_token_loja" value="<?=e(Secrets::mask($config['vsm_client_token_loja']??''))?>"><div class="form-text">Vai como <code>clientTokenLoja</code> na query do pedido.</div></div>

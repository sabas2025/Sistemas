...
$versao = 'V3';
$rotaExecutar = 'tiny-v3-homologacao-executar';
$tituloAuth = '🔐 OAuth Tiny V3';
$authLista = '<ul class="small mb-0 mt-2">'
  . '<li>Base API: <code>'.e($cfg['tiny_v3_url'] ?? ($cfg['tiny_v3_base_api'] ?? 'https://api.tiny.com.br/public-api/v3')).'</code></li>'
  . '<li>Client ID: '.(trim((string)($cfg['tiny_v3_client_id'] ?? ''))!=='' ? '✔ informado' : '<span class="text-danger">✖ ausente</span>').'</li>'
  . '<li>Redirect URI: '.(trim((string)($cfg['tiny_v3_redirect_uri'] ?? ''))!=='' ? '✔ informado' : '<span class="text-danger">✖ ausente</span>').'</li>'
  . '<li>Access Token: '.(trim((string)($cfg['tiny_v3_access_token'] ?? ''))!=='' ? '✔ informado' : '<span class="text-warning">verificar serviço OAuth</span>').'</li>'
  . '<li>Última validação: '.e($ultimo['criado_em'] ?? 'sem histórico').'</li>'
  . '</ul>';
$pedidoHint = 'Pedido do ambiente Tiny V3 selecionado.';
?>

<div class="card-soft mb-3 border border-success-subtle">
  <div class="d-flex flex-wrap justify-content-between gap-3 align-items-start">
    <div>
      <h4 class="mb-1"><i class="bi bi-check2-circle"></i> Tiny V3 - Homologação</h4>
...
<div class="row g-3 mt-1">
  <div class="col-xl-8">
    <div class="card-soft h-100">
      <h5>▶ Executar Homologação Tiny V3</h5>
...
<div class="row g-3 mt-1">
  <div class="col-lg-6">
    <div class="card-soft h-100">
      <h5>📊 Homologação de Estoque Tiny V3 x VSM</h5>
      ...
      <table class="table table-sm mb-0 mt-2">
        ...
        <tr><th>Tiny V3</th><td><?=isset($estoque['tiny_saldo']) && $estoque['tiny_saldo'] !== null ? e((string)$estoque['tiny_saldo']) : '—'?></td></tr>
